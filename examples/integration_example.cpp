...
int main(int argc, char *argv[])
{
    ...
    // Вариант В - переиспользовать Modbus-соединение, которое хост-GUI уже
    // создало и открыло само (типичный случай - MainWindow::init() со своим
    // QModbusRtuSerialMaster/Client) - НЕ открывать своё второе соединение к
    // тому же COM-порту. См. README, "Переиспользование существующего
    // Modbus-соединения":
    // modbus.configureExternalClient(mainWindow->modbusDevice());
    ...
    // Для вариантов А/Б connectDevice() реально открывает соединение. Для
    // варианта В (внешний клиент) он ничего не открывает сам - только
    // сообщает уже актуальное состояние GUI-соединения, поэтому в этом
    // варианте здесь важно, чтобы modbusDevice уже был подключен к этому
    // моменту (например, MainWindow подключает его раньше, при старте
    // приложения) - иначе engine.setModbusClient() ниже не вызовется, и
    // диспетчер останется в режиме симуляции до перезапуска этого кода.
