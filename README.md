...
- Биты 8-9 (Battery First / Load First) — режим распределения мощности СЭС,
  ...
  Диспетчер держит этот бит **всегда = Load First**. Сравнение "заряд vs
  продажа" (как в шаге 2 эвристики/MILP) к этому полю неприменимо — в
  ZeroExport продажи и так почти нет (случай C) или она жёстко ограничена
  излишками (случай B). Реальный выбор поля — "заряд vs нагрузка", и здесь
  Battery First хуже: при нехватке солнца инвертор **в обход диспетчера**
  тянет из сети мощность одновременно и на заряд СНЭ, и на нагрузку — то
  есть может покупать электроэнергию для заряда именно тогда, когда то же
  солнце можно было бы напрямую пустить на нагрузку. При Load First сеть
  привлекается только туда, где без неё не обойтись (нагрузка), а заряд от
  сети остаётся полностью под контролем диспетчера — через явный бит Grid
  Charge Enabled и плановый предел тока (2041).
