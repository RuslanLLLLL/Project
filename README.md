...
на полпути; излом скруглён с радиусом `kConnectionCornerRadius` (по умолчанию
14px — константа рядом с `kConnectionLineWidth`/`kParticleRadius` в начале
`EnergyMimicWidget.cpp`, поставьте 0 для острого угла без скругления).
Анимированные частицы, показывающие направление и интенсивность потока,
следуют по той же ломаной линии. Точки входа связей в инвертор со стороны
"верхних" (СЭС, Сеть) и "нижних" (СНЭ, Нагрузка) узлов дополнительно разнесены
по вертикали внутри карточки инвертора, чтобы связи с одной стороны схемы не
