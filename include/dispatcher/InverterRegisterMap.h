...
//   бит 6-7: Gen Signal (дистанционный запрос запуска генератора). Диспетчер
//            держит этот бит ВСЕГДА =1, пока разрешение на генератор в
//            принципе включено (DispatchPermissions::m_genEnable) - реальный
//            запуск/останов выполняет сам инвертор по собственным порогам SOC
//            (регистр 2107 - запуск, 2106 - останов), которые диспетчер не
//            трогает. Диспетчер НЕ выставляет этот бит по своему плановому
//            genRunning на текущий интервал - решение целиком за автоматикой
//            инвертора.
//   бит 8-9: Battery First / Load First - режим распределения мощности СЭС
//            в режиме нулевого экспорта (ZeroExport, регистр 2100):
//              Load First (00)    - солнце сначала на нагрузку, излишки на
//                                   заряд СНЭ; при нехватке солнца сеть
//                                   докрывает нагрузку.
//              Battery First (01) - солнце сначала на заряд СНЭ, излишки на
//                                   нагрузку; при нехватке солнца сеть
//                                   одновременно докрывает и СНЭ, и нагрузку.
//            Вне ZeroExport (режим Sell First, регистр 2100) поле не
//            действует. Диспетчер держит его ВСЕГДА = Load First (00) -
//            здесь сравнение "заряд vs продажа" (как в шаге 2 эвристики)
//            неприменимо, т.к. в ZeroExport продажи и так почти нет; реальный
//            выбор поля - "заряд vs нагрузка", и в нём Battery First хуже:
//            при нехватке солнца инвертор В ОБХОД диспетчера тянет из сети
//            мощность одновременно на заряд СНЭ и на нагрузку, тогда как
//            заряд от сети и так полностью управляется диспетчером явно
//            (Grid Charge Enabled + предел тока 2041).
